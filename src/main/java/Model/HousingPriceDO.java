...
public class HousingPriceDO {
    ...
    public HousingPriceDO(String[] record) {
        this.setPrecinctID(Long.valueOf(record[0]));
    ...
    private HashMap<Date, Double> HPI;
    ...
    public HashMap<Date, Double> getHPI() {
        return HPI;
    }
    ...
    public void setHPI(String[] HPIRecord) {
        ...
        for (int i = 0; i < timestamps.length; i++) {
            String[] dm = timestamps[i].split("/");
            data.put(new Date(Integer.parseInt(dm[1]) + 100, Integer.parseInt(dm[0]) - 1, 1), Double.valueOf(HPIRecord[i+4]));
        }
        this.HPI = data;
    }

    public Double getHPIonDate(Date date) {
        date.setDate(1);
